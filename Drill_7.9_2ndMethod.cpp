...
Token Token_stream::get(){
    ...
    switch (ch){
        case '=':    // for "print"
        case 'q':    // for "quit"
        case '(': case ')': case '+': case '-': case '*': case '/': case 'k': case 's': case 'p': case ',':
        ...
        case 's':
       	{
            t = ts.get();
            if ( t.kind != '(' ) throw("Error : '(' expected ");
			double d = expression();		// Error expected if after '(' there is '-'  ... Solution s( 0 - number )...
            if(d < 0) throw("Error : Expression is negstive in Square Root");
            t = ts.get();
            if ( t.kind != ')' ) throw("Error : ')' expected ");
            return sqrt(d);
        }
        case 'p':
       	{
            t = ts.get();
            if ( t.kind != '(' ) throw("Error : '(' expected ");
            power.push_back(expression());
        	t = ts.get();
        	if ( t.kind != ',' ) throw("Error : Bad number of function arguments ...  OR  ',' expected....");
        	power.push_back(expression());
        	t = ts.get();
            if ( t.kind != ')' ) throw("Error : ')' expected ");
            return pow(power[0], power[1]);
        }
        default:
