...
try {
	...
	for (int p = 0 ; p < processed_points.size(); p = p+2 ){
		cout<<"(X, Y) = (" << processed_points[p] << ", " << processed_points[p+1] << ")\n";
}

		for (int p = 0 ; p < processed_points.size(); p++){
			//processed_points[5] = 12345;
		if(processed_points[p] != original_points[p]){
